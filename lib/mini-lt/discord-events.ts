...
/**
 * Discord event operations called after authorization by the caller.
 */
...
export async function createWeekEvent(weekId: string): Promise<void> {
  ...
  const startDateUTC = new Date(
    Date.UTC(year_val, month_val, day_val, 12, 0, 0),
  );
...
export async function syncWeekEventDescription(weekId: string): Promise<void> {
  ...
  const eventId = weekData.discordEventId;
  if (!eventId) throw new Error("Discord event not found");
  const description = buildEventDescription(weekData);
