...
import { Users, CalendarDays, Pencil, Share2, Check, Circle } from "lucide-react"
...
function getProfileCompletionItems(member: Record<string, unknown> | null): CompletionItem[] {
  if (!member) return []
  return [
...
export default async function InternalPage() {
  ...
  const items = getProfileCompletionItems(member as Record<string, unknown> | null)
  const completion = items.length > 0 ? Math.round((items.filter(i => i.done).length / items.length) * 100) : 0
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="lg:col-span-1 animate-spring-up stagger-tight-2 fill-mode-backwards">
          ...
          <CardContent className="space-y-3">
            ...
            {completion < 100 && (
              <Link href="/internal/profile/edit" className="inline-block text-xs text-purple-600 dark:text-purple-400 hover:underline mt-1">
                プロフィールを編集 →
              </Link>
            )}
          </CardContent>
            ...
            <Link key={href} href={href} className={`stagger-tight-${i + 3} animate-spring-up fill-mode-backwards`}>
              <Card className="h-full hover:shadow-xl hover:-translate-y-1 active:scale-[0.98] transition-all duration-300 cursor-pointer group border-transparent hover:border-purple-200 dark:hover:border-purple-800">
                <CardContent className="flex items-start gap-4 p-5">
                  ...
                  <div>
                    <p className="font-semibold text-foreground group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-colors">{label}</p>
