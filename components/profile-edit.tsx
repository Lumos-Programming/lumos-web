...
const PROFILE_FIELDS = Object.keys(FIELD_LABELS) as Array<
  keyof Omit<Profile, "visibility" | "role" | "year" | "enrollments">
>;
...
export default function ProfileEdit() {
  ...
  const [undergradEnrollmentType, setUndergradEnrollmentType] = useState<
    EnrollmentType | ""
  >("");
      ...
      .then((data) => {
        if (data && !data.error) {
          ...
          if (typeof data.allowPublic === "boolean") {
            ...
            const hasPublic = VISIBILITY_FIELD_KEYS.some(
              (k) => vis[k] === "public",
            );
          ...
          setEnrollmentType(
            (currentEnrollment?.enrollmentType as EnrollmentType) ?? "",
          );
          setTransferYear(currentEnrollment?.transferYear ?? "");
          setGraduationYear(currentEnrollment?.graduationYear ?? "");
          const undergradEnrollment = data.enrollments?.find(
            (e: { isCurrent?: boolean }) => !e.isCurrent,
          );
          if (
            data.memberType === "院生" ||
            (data.memberType === "卒業生" &&
              (GRADUATE_SCHOOLS as readonly string[]).includes(
            ...
            if (undergradEnrollment) {
              ...
              setUndergradEnrollmentType(
                (undergradEnrollment.enrollmentType as EnrollmentType) ?? "",
              );
          ...
          if (
            data.memberType &&
            (MEMBER_TYPES as readonly string[]).includes(data.memberType)
          )
            setMemberType(data.memberType as MemberType);
          ...
          setInitialSnapshot(
            JSON.stringify({
              ...
              undergradEnrollmentType:
                undergradEnrollment?.enrollmentType ?? "",
  ...
  const isDirty =
    !loading && initialSnapshot !== "" && currentSnapshot !== initialSnapshot;
        ...
        body: JSON.stringify({
          ...
          ...(() => {
            const entries = [
              ...
              ...((memberType === "院生" ||
                (memberType === "卒業生" &&
                  (GRADUATE_SCHOOLS as readonly string[]).includes(faculty))) &&
              hasUndergrad &&
              undergradFaculty
                ? [
                    {
                      ...
                      ...(undergradTransferYear
                        ? { transferYear: undergradTransferYear }
                        : {}),
                      isCurrent: false,
                    },
                  ]
  ...
  const handleBannerCropConfirm = useCallback(async () => {
    ...
    try {
      const blob = await cropAndResizeImage(bannerCropSrc, bannerCroppedArea, {
        maxSize: 1200,
      });
      const formData = new FormData();
  ...
  const discordAvatarUrl = useMemo(
    () => resolveAvatar(discordId, discordAvatarHash),
    [resolveAvatar, discordId, discordAvatarHash],
  );
  ...
  const buildSnsEntries = useCallback(
  ...
  const internalSns = useMemo(
    () => buildSnsEntries("internal"),
    [buildSnsEntries],
  );
  const externalSns = useMemo(
    () => buildSnsEntries("public"),
    [buildSnsEntries],
  );
      ...
      <Card>
        <CardContent className="pt-6 space-y-6">
          ...
          <div className="space-y-4 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
            ...
            <p className="text-xs text-muted-foreground">
              プロフィールカード上部に表示される背景画像です。未設定の場合はグラデーションが表示されます。
            </p>
          ...
          <div className="space-y-4 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                ...
                {MEMBER_TYPES.map((type) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setProfile((p) => ({
                        ...p,
                        currentOrg: cached?.currentOrg ?? "",
                      }));
                      setHasUndergrad(cached?.hasUndergrad ?? null);
                      setUndergradFaculty(cached?.undergradFaculty ?? "");
                      setUndergradAdmissionYear(
                        cached?.undergradAdmissionYear ?? "",
                      );
                      setUndergradEnrollmentType(
                        cached?.undergradEnrollmentType ?? "",
                      );
                      setUndergradTransferYear(
                        cached?.undergradTransferYear ?? "",
                      );
                    }}
                    className={[
            ...
            {memberType &&
              memberType !== "卒業生" &&
              (() => {
                const { label, note, options } =
                  getSchoolYearOptions(memberType);
                const now = new Date();
                const fiscalYear =
                  now.getMonth() >= 3
                    ? now.getFullYear()
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex gap-2">
                        <select
                          ...
                          onChange={(e) =>
                            setUndergradAdmissionYear(e.target.value)
                          }
                          className="flex-1 border rounded-md px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-ring bg-white dark:bg-gray-800 dark:text-gray-100 border-input dark:border-gray-700"
                        >
                          <option value="">年度を選択</option>
            ...
            {PROFILE_FIELDS.map((key) => {
              ...
              return (
                <div
                  key={key}
                  className={
                    key === "bio" ? "md:col-span-2 space-y-2" : "space-y-2"
                  }
                >
                  {key !== "bio" && (
          ...
          <div className="rounded-xl border-2 border-purple-200 dark:border-purple-800/60 p-4 space-y-4 mt-2">
            <div>
              <h3 className="text-base font-semibold bg-gradient-to-r from-purple-700 to-indigo-600 bg-clip-text text-transparent dark:from-purple-400 dark:to-indigo-400">
                公開設定
              </h3>
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800/50 p-3 space-y-1.5 text-xs text-gray-600 dark:text-gray-400">
              <div className="flex items-start gap-2">
                <span className="inline-block px-1.5 py-0.5 rounded-full bg-gray-500 text-white font-medium flex-shrink-0">
                  非公開
                </span>
                <span>
                  自分だけが閲覧できます。他のメンバーにも表示されません。
                </span>
              </div>
              <div className="flex items-start gap-2">
                <span className="inline-block px-1.5 py-0.5 rounded-full bg-indigo-600 text-white font-medium flex-shrink-0">
                  内部のみ
                </span>
                <span>
                  Lumosメンバーだけが閲覧できます。外部向けHPには表示されません。
                </span>
              </div>
              <div className="flex items-start gap-2">
                <span className="inline-block px-1.5 py-0.5 rounded-full bg-green-600 text-white font-medium flex-shrink-0">
                  外部公開
                </span>
                <span>
                  LumosのHP（公開サイト）にも表示されます。誰でも閲覧できます。
                </span>
              </div>
            </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-end gap-6 text-xs text-gray-400 dark:text-gray-500 pr-1">
                <span>非公開</span>
                <span>内部のみ</span>
                <span className={allowPublic ? "" : "opacity-30"}>
                  外部公開
                </span>
              </div>
              {VISIBILITY_DISPLAY_KEYS.map((key) => {
                ...
                const visibilityValue = isCombinedName
                  ? profile.visibility.lastName === profile.visibility.firstName
                    ...
                    : "internal"
                  : (profile.visibility[
                      key as keyof typeof profile.visibility
                    ] ?? "private");
                return (
                  <div
                    key={key}
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-6 pb-4">
          <div className="flex items-center justify-between rounded-xl bg-gradient-to-r from-gray-900 to-gray-800 dark:from-gray-800 dark:to-gray-700 px-5 py-3 shadow-2xl shadow-black/20">
            <div className="flex items-center gap-2.5 text-sm text-gray-200">
              <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-yellow-500 text-white text-xs font-bold shrink-0">
                !
              </span>
              未保存の変更があります
            </div>
