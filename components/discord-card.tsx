...
import SubDiscordSettings, {
  type SubAccount,
} from "@/components/sub-discord-settings";
...
interface Props {
  /** ログイン中のメインアカウントの Discord ハンドル */
  discordUsername: string;
  /** 連携済みのサブアカウント (未連携なら null) */
  subAccount: SubAccount | null;
}

/**
 * SNS連携タブの Discord カード。
 * メインの Discord は常に連携済みなので操作は無く、その下にサブアカウントの
 * 行を持つ。Discord まわりの表示をこのカードに閉じ込めておく。
 */
export default function DiscordCard({ discordUsername, subAccount }: Props) {
  return (
    <div className="p-4 border rounded-xl bg-card hover:shadow-md transition-all duration-200">
      ...
      <div className="mt-4 pt-4 border-t border-border/60">
        <SubDiscordSettings subAccount={subAccount} />
      </div>
    </div>
  );
}
