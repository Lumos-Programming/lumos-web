  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-[#0d0f1a] overflow-hidden">
      ...
      {[...Array(6)].map((_, i) => (
        <div
          ...
          style={{
            width: `${3 + (i % 3) * 2}px`,
            height: `${3 + (i % 3) * 2}px`,
            background: i % 2 === 0 ? 'rgba(88,101,242,0.3)' : 'rgba(130,120,200,0.25)',
            left: `${12 + i * 14}%`,
            top: `${20 + ((i * 19) % 55)}%`,
      ...
      <div
        ...
        style={{
          background: "radial-gradient(circle, rgba(88,101,242,0.08) 0%, rgba(130,120,200,0.06) 40%, transparent 70%)",
        }}
      />
      ...
      <div className="relative z-10 flex flex-col items-center w-full max-w-md px-6">
        ...
        <p className="mt-6 text-[#c8cae0] text-sm leading-relaxed text-center animate-[loginFadeIn_0.6s_ease_0.8s_both]">
          Lumosのメンバー登録を行います。<br />
          まず本人確認のため、<strong className="text-white">Discord</strong>でログインしてください。
        </p>
        ...
        <div className="mt-5 w-full flex gap-3 rounded-lg border border-[#5865F2]/30 bg-[#5865F2]/10 px-4 py-3 animate-[loginSlideIn_0.5s_cubic-bezier(0.25,0.46,0.45,0.94)_1.1s_both]">
          <svg className="w-5 h-5 text-[#5865F2] flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
            <circle cx={12} cy={12} r={10} />
            <line x1={12} y1={16} x2={12} y2={12} />
            <line x1={12} y1={8} x2={12.01} y2={8} />
          </svg>
          <p className="text-sm text-[#c8cae0] leading-relaxed">
            Discordサーバー「<strong className="text-white">Lumos-プログラミングサークル</strong>」に参加済みのアカウントでログインしてください。
          </p>
        </div>
      ...
      <style jsx>{`
        @keyframes loginOrbit {
          ...
          10%  { opacity: 0.6; }
          50%  { transform: translateY(-50px) translateX(20px) scale(1.4); opacity: 0.4; }
          90%  { opacity: 0.6; }
