  ...
  if (entry.platform === "line") {
    return (
      <>
        ...
        {showLinePopup && (
          <LinePopup entry={entry} onClose={() => setShowLinePopup(false)} />
        )}
      </>
    );
  }
...
export function socialToSnsEntries(social?: SocialInput): SnsEntry[] {
  ...
  if (social.line) {
    entries.push({
      platform: "line",
      username: social.line,
      avatarUrl: social.lineAvatar,
    });
  }
  if (social.x) {
    const username = social.x
