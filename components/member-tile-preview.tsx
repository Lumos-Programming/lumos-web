...
export function InternalTilePreview({
  data,
  label,
}: {
  data: MemberTileData;
...
export function ExternalTilePreview({
  data,
  label,
  allowPublic,
}: {
      ...
      <div className="relative rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 overflow-hidden">
        ...
        {!allowPublic && (
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="bg-gray-800/80 text-white text-sm font-semibold px-3 py-1.5 rounded-lg">
              HP掲載 OFF
            </span>
          </div>
        )}
...
function DetailPreview({
  ...
  const dept =
    data.memberType === "卒業生" && data.currentOrg
      ? data.currentOrg
      : data.department;
          ...
          <div className="flex gap-3 p-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                {data.memberType && (
                  <span
                    className={`inline-block text-[10px] font-medium px-1.5 py-0.5 rounded-full ${getMemberTypeBadgeClass(data.memberType)}`}
                  >
...
export function DetailPreviewPanel({
  ...
  const [detailSide, setDetailSide] = useState<"internal" | "external">(
    "internal",
  );
  return (
...
export function MemberPreviewToggle({
  internalData,
  externalData,
  allowPublic,
}: PreviewToggleProps) {
