...
const SNS_BRAND: Record<SnsEntry["platform"], { label: string; bg: string; text: string; icon: (props: { className?: string }) => React.ReactNode }> = {
  github: {
    label: "GitHub",
    bg: "bg-[#24292f] dark:bg-[#333]",
    text: "text-white",
    icon: ({ className }) => (
...
function SnsChip({ entry }: { entry: SnsEntry }) {
  ...
  return (
    <div className={`inline-flex items-center gap-1.5 rounded-full pl-1 pr-2.5 py-1 ${brand.bg} ${brand.text}`}>
      {entry.avatarUrl ? (
        <div className="w-5 h-5 relative rounded-full overflow-hidden flex-shrink-0 ring-1 ring-white/20">
