...
import type { SubAccount } from "@/components/sub-discord-settings";
...
interface Props {
  ...
  errorMessage?: string;
  /** 連携済みのサブアカウント (Discord カード内に表示する) */
  ...
  return (
    <div className="flex items-center justify-between gap-2 p-4 border rounded-xl bg-card hover:shadow-md transition-all duration-200">
      <div className="flex items-center gap-3 min-w-0">
        <div
          className={`w-10 h-10 ${brandColor} rounded-xl flex items-center justify-center text-white shrink-0`}
        >
          {icon}
        </div>
        <div className="min-w-0">
          ...
          {isConnected ? (
            <p className="text-xs text-muted-foreground truncate">
              {prefixAt ? `@${connectedUser}` : connectedUser}
            </p>
          ) : (
            ...
            {isSet && !editing ? (
              <p className="text-xs text-muted-foreground">
                ...
                <span className="inline-block float-right">
                  {displayUrl.replace(/^linkedin\.com\/in\//, "")}
                </span>
              </p>
            ) : !editing ? (
...
export default function SnsSettings({
  ...
  return (
    <div className="space-y-3">
      ...
      <DiscordCard discordUsername={discordUsername} subAccount={subAccount} />
